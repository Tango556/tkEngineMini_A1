...
Texture2D<float4> sceneTexture : register(t0); // シーンテクチャ
...
PS_BlurInput VSXBlur(VSInput In)
{
    ...
    //基準テクセルから+3テクセルずらすためのオフセットを計算する
...
PS_BlurInput VSYBlur(VSInput In)
{
    ...
    //基準テクセルからU座標を+1テクセルずらすためのオフセットを計算する
    Out.tex0.xy = float2(0.0, 1.0f / texSize.y);
    
    //基準テクセルから+3テクセルずらすためのオフセットを計算する
    Out.tex1.xy = float2(0.0, 3.0f / texSize.y);
    
    //基準テクセルからU座標を+5テクセルずらすためのオフセットを計算する
    Out.tex2.xy = float2(0.0, 5.0f / texSize.y);
    
    //基準テクセルからU座標を+7テクセルずらすためのオフセットを計算する
    Out.tex3.xy = float2(0.0, 7.0f / texSize.y);

    //基準テクセルからU座標を+9テクセルずらすためのオフセットを計算する
    Out.tex4.xy = float2(0.0, 9.0f / texSize.y);

    //基準テクセルからU座標を+11テクセルずらすためのオフセットを計算する
    Out.tex5.xy = float2(0.0, 11.0f / texSize.y);

    //基準テクセルからU座標を+13テクセルずらすためのオフセットを計算する
    Out.tex6.xy = float2(0.0, 13.0f / texSize.y);
    
    //基準テクセルからU座標を+15テクセルずらすためのオフセットを計算する
    Out.tex7.xy = float2(0.0, 15.0f / texSize.y);
...
float4 PSBlur(PS_BlurInput In) : SV_Target0
{
    ...
    Color += weight[1].y * sceneTexture.Sample(Sampler, In.tex6.xy);
    Color += weight[1].y * sceneTexture.Sample(Sampler, In.tex7.xy);
    ...
    Color += weight[1].y * sceneTexture.Sample(Sampler, In.tex6.zw);
    Color += weight[1].y * sceneTexture.Sample(Sampler, In.tex7.zw);
