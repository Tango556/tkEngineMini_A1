...
{
	m_modelInitData[enModel].m_tkmFilePath = "Assets/modelData/unityChan.tkm";
	m_modelInitData[enModel].m_fxFilePath = "Assets/shader/model.fx";
	m_modelInitData[enModel].m_expandConstantBuffer = &lig;
	m_modelInitData[enModel].m_expandConstantBufferSize = sizeof(lig);
	m_modelInitData[enModel].m_collorBufferFormat[0] = DXGI_FORMAT_R32G32B32A32_FLOAT;
	m_modelInitData[enShadow].m_tkmFilePath = "Assets/modelData/unityChan.tkm";
	m_modelInitData[enShadow].m_fxFilePath = "Assets/shader/SampleDepthShadow.fx";
	m_modelInitData[enShadow].m_expandConstantBuffer = &lig;
	m_modelInitData[enShadow].m_expandConstantBufferSize = sizeof(lig);
...
void Player::Update(Light& lig)
{
	...
	SetPosition(GetPositon() + m_controller.Move());
